(() => {
  ...
  cargar(base + '/_ds_bundle.js')
    ...
    .then(() => new Promise((ok) => {
      (function esperarApp() {
        if (window.AppIntraMovil) return ok();
    ...
    .then(() => {
      ...
      nodo._montado = 1;
      window.ReactDOM.createRoot(nodo).render(window.React.createElement(window.AppIntraMovil));
    })
    .catch((e) => console.error('ds-base.js: no se pudo montar AppIntraMovil —', e));
})();
